	...
	if (random < luck) {
		if (gauge < 50) {
			...
			if (stage === 3) {
				...
				await User.update({ gold: gold + parseInt(((typing / 100) * 70).toFixed(0)) * 2, exp: exp + patience / 10 }, { where: { id: user_idx } });
			} else if (stage === 4) {
				console.log('4 스테이지 유저 골드 2');
				await User.update({ gold: gold + parseInt(((typing / 100) * 70).toFixed(0)) * 2, exp: exp + patience / 50 }, { where: { id: user_idx } });
			} else if (stage === 5) {
				console.log('5 스테이지 유저 골드 2');
				await User.update({ gold: gold + parseInt(((typing / 100) * 70).toFixed(0)) * 2, exp: exp + patience / 100 }, { where: { id: user_idx } });
			}
		} else {
			console.log('게이지 50이상');
			if (stage === 3) {
				console.log('3 스테이지 유저 골드 2');
				await User.update({ gold: gold + typing + typing, exp: exp + patience / 10 }, { where: { id: user_idx } });
			} else if (stage === 4) {
				console.log('4 스테이지 유저 골드 2');
				await User.update({ gold: gold + typing + typing, exp: exp + patience / 50 }, { where: { id: user_idx } });
			} else if (stage === 5) {
				console.log('5 스테이지 유저 골드 2');
				await User.update({ gold: gold + typing + typing, exp: exp + patience / 100 }, { where: { id: user_idx } });
			}
		}
	} else {
		if (gauge < 50) {
			...
			if (stage === 3) {
				...
				await User.update({ gold: gold + parseInt(((typing / 100) * 70).toFixed(0)), exp: exp + patience / 10 }, { where: { id: user_idx } });
			} else if (stage === 4) {
				console.log('4 스테이지 유저');
				await User.update({ gold: gold + parseInt(((typing / 100) * 70).toFixed(0)), exp: exp + patience / 50 }, { where: { id: user_idx } });
			} else if (stage === 5) {
			...
			if (stage === 3) {
				console.log('3 스테이지 유저');
				await User.update({ gold: gold + typing, exp: exp + patience / 10 }, { where: { id: user_idx } });
			} else if (stage === 4) {
				console.log('4 스테이지 유저');
				await User.update({ gold: gold + typing, exp: exp + patience / 50 }, { where: { id: user_idx } });
			} else if (stage === 5) {
				console.log('5 스테이지 유저');
				await User.update({ gold: gold + typing, exp: exp + patience / 100 }, { where: { id: user_idx } });
			}
		}
	}
